...
#define LCD_E(x)              ((x) ? (FIO1SET = PIN_E)  : (FIO1CLR = PIN_E) );

/* pin RW setting to 0 or 1                                                   */
#define LCD_RW(x)             ((x) ? (FIO1SET = PIN_RW) : (FIO1CLR = PIN_RW));

/* pin RS setting to 0 or 1                                                   */
#define LCD_RS(x)             ((x) ? (FIO1SET = PIN_RS) : (FIO1CLR = PIN_RS));

/* Reading DATA pins                                                          */
#define LCD_DATA_IN           ((FIO1PIN >> 24) & 0xF)

/* Writing value to DATA pins                                                 */
#define LCD_DATA_OUT(x)       FIO1CLR = PINS_DATA; FIO1SET = (x & 0xF) << 24;

/* Setting all pins to output mode                                            */
#define LCD_ALL_DIR_OUT       FIO1DIR  |=  PINS_CTRL | PINS_DATA;

/* Setting DATA pins to input mode                                            */
#define LCD_DATA_DIR_IN       FIO1DIR &= ~PINS_DATA;

/* Setting DATA pins to output mode                                           */
#define LCD_DATA_DIR_OUT      FIO1DIR |=  PINS_DATA;
...
static char lcd_read_status (void)
{
    char status;
    LCD_DATA_DIR_IN
    LCD_RS(0)
...
static char wait_while_busy (void)
{
    char status;
...
void set_cursor (char column, char line)
{
    char address;
...
void lcd_print (char const *string)
{
    while (*string)
    {
        lcd_putchar (*string++);
